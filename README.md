# AI Contract Analysis & Risk Assistant
...
An automated legal assistant that scans contract agreements, extracts key clauses (indemnity, termination, governing law), and flags potential liability risks.

---

## Key Features

- **Automated**: clause extraction for indemnity, liability limitation, and termination
- **Liability**: risk assessment scoring contracts on severity scale
- **Non-standard**: clause detection comparing against standard legal baselines
- **Executive**: summary generation with critical action items for legal counsel
- **Built-in**: compliance disclaimer guardrails
...
```mermaid
flowchart LR
    Contract([Contract Text]) --> Parser[Clause Segmenter]
    Parser --> ClauseDB[(Clause Catalog)]
    ClauseDB --> RiskScanner[Liability Risk Scanner]
    RiskScanner --> RiskReport[Risk Assessment & Clause Matrix]
```
...
├── tests/
│   ├── __init__.py
│   └── test_contract_analysis.py   # Automated test suite
...
1. **Clone the repository:**
   ```bash
   git clone https://github.com/erhatechnologiesai/ai-contract-analysis.git
...
|---|---|---|
| `POST` | `/analyze-contract` | Extract contract clauses and evaluate liability risk levels |

### Example Request

```bash
curl -X POST http://127.0.0.1:8000/analyze-contract -H "Content-Type: application/json" -d '{"contract_text": "This agreement terminates immediately without cause. Party A assumes unlimited liability for damages."}'
```
...
```bash
python -m unittest tests/test_contract_analysis.py
```
